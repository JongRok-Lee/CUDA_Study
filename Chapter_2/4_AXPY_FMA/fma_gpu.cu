...
template<typename PREC>
__global__ void kernelVecAddFMAGPU(PREC vecC[], const PREC vecA[], const PREC vecB[], const int size, const float a) {
  unsigned int id = blockIdx.x * blockDim.x + threadIdx.x;
  if (id < size) {
    // vecC[id] = a * vecA[id] + vecB[id];
    vecC[id] = fmaf(a, vecA[id], vecB[id]);
  }
}
...
int main()
{
  ...
  kernelVecAddFMAGPU<<<(SIZE + 1023) / 1024, 1024>>>(d_vecZ, d_vecX, d_vecY, SIZE, saxpy_a);
